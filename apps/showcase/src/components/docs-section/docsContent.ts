import { installSnippets, PACKAGE_DIRS, PACKAGE_SLUGS, usageSnippets } from "./snippets";
...
export const DOCS_SECTIONS: DocSectionDef[] = [
    ...
    blocks: framework => [
      {
        kind: "badges",
        items: [
          { label: "npm", href: `https://www.npmjs.com/package/@brustack/${PACKAGE_SLUGS[framework]}` },
          { label: "bundle size ↗", href: `https://bundlephobia.com/package/@brustack/${PACKAGE_SLUGS[framework]}` },
          {
            label: "MIT License",
  ...
  },
  {
    ...
    blocks: framework => [{ kind: "code", entries: usageSnippets[framework] }],
  },
  {
    id: "styling",
    eyebrow: "03 · Styling",
    ...
    blocks: (framework) => {
      const blocks: DocBlock[] = [
        ...
        { kind: "code", entries: [{ file: "style.css", code: cssVariablesCode }] },
        {
          kind: "callout",
        ...
        { kind: "code", entries: [{ file: "tailwind.config.js", code: tailwindConfigCode }] },
      ];

      if (framework === "React" || framework === "Next.js") {
        blocks.push(
          { kind: "subhead", text: "Styled-components (or any CSS-in-JS)" },
          { kind: "code", entries: [{ file: "App.tsx", code: styledComponentsCode }] },
        );
      }
    ...
    blocks: () => [
      {
        ...
        rows: [
          ...
          ["<code>fade</code> <span class=\"muted\">(default)</span>", "<code>'400ms'</code>", "<code>'ease'</code>"],
      ...
      { kind: "code", entries: [{ file: "example", code: customThemesExampleCode }] },
      {
        kind: "callout",
    ...
    blocks: () => [
      {
        kind: "table",
        rows: [
          ["<code>theme</code>", "Current resolved theme: <code>'light'</code>, <code>'dark'</code>, or a custom name"],
          ["<code>mode</code>", "Current preference, including <code>'system'</code>"],
          ["<code>isAnimating</code>", "<code>true</code> while a transition is running"],
          ["<code>themes</code>", "All registered names: <code>['light','dark','system',...custom]</code>"],
          ["<code>toggleTheme(event?)</code>", "Switch between light and dark"],
          ["<code>setTheme(mode, event?)</code>", "Set a specific mode or custom theme"],
        ],
      },
    ],
  },
  {
    ...
    blocks: () => [
      { kind: "code", entries: [{ file: "vite.config.ts", code: viteConfigCode }] },
      {
        kind: "callout",
